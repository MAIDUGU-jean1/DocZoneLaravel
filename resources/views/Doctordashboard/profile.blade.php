<x-app>
    ...
    <body
        class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="w-full max-w-4xl bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row items-center sm:items-start space-y-6 sm:space-y-0 sm:space-x-6">
                ...
                @if ($doctor->role === 'doctor')
                    <img src="{{ asset('storage/' . $doctor->profile_picture) }}" alt="Dr. John Doe"
                        class="w-50 h-30 sm:w-32 sm:h-32 rounded object-cover shadow-lg" />
                @endif

                <!-- Doctor Info -->
                <div class="text-center sm:text-left">
                    @if ($doctor->role === 'doctor')
                        <h1 class="text-2xl sm:text-3xl font-bold">Dr. {{ $doctor?->name }}</h1>
                    @endif
